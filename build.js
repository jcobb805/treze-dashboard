...
<div class="content">
  ...
  <div class="tab-content" id="tab-analysis">
    ...
    <div class="table-wrap" style="max-height:500px;overflow-y:auto">
      <table><thead><tr>
        <th>Month</th><th>Active Notes</th><th>N/R Income</th><th>Balloon Payoffs</th><th>FUB Interest</th><th>FUB Principal</th><th>Extra Paydown</th><th>Total Debt Service</th><th>Net Cash Flow</th><th>FUB Balance</th>
...
window.renderAnalysis = function() {
  ...
  for (let m = 0; m < 49; m++) { // ~4 years
    ...
    // Balloon payoffs received this month
    const balloonNotes = NOTES.filter(n => {
      const balloon = new Date(n.balloonDate);
      return balloon.getFullYear() === monthDate.getFullYear() && balloon.getMonth() === monthDate.getMonth();
    });
    const balloonIncome = balloonNotes.reduce((s, n) => s + n.currentBalance, 0);
    ...
    const netCf = nrIncome + balloonIncome - monthlyInterest - basePrincipal - actualExtra;

    totalNetCf += netCf;
    totalNrIncome += nrIncome + balloonIncome;
    ...
    rows.push({
      ...
      balloonCount: balloonNotes.length,
      balloonIncome,
      nrIncome,
    ...
    nrIncomeData.push(Math.round(nrIncome + balloonIncome));
  ...
  // KPIs
  const totalBalloonIncome = rows.reduce((s,r) => s + r.balloonIncome, 0);
  const avgNetCf = totalNetCf / rows.length;
  // PV10: discount each month's net CF at 10% annual rate
  const monthlyDiscount = Math.pow(1 + 0.10, 1/12);
  let pv10 = 0;
  rows.forEach((r, i) => { pv10 += r.netCf / Math.pow(monthlyDiscount, i + 1); });
  const totalPrincipalPaid = FUB.balanceAsOf - fubBal;
  document.getElementById('analysisKpi').innerHTML = [
    {label:'Total Net Cash Flow',value:'$'+Math.round(totalNetCf/1000).toLocaleString()+'K',cls:totalNetCf>=0?'green':'red'},
    {label:'PV10 (Net Cash Flow)',value:'$'+Math.round(pv10/1000).toLocaleString()+'K',cls:pv10>=0?'green':'red'},
    {label:'Avg Monthly Net CF',value:'$'+Math.round(avgNetCf).toLocaleString(),cls:avgNetCf>=0?'green':'red'},
    {label:'Balloon Payoffs (2030)',value:'$'+(totalBalloonIncome/1000000).toFixed(1)+'MM',cls:'green'},
  ...
  document.getElementById('analysisBody').innerHTML = rows.map((r,i) => {
    ...
    return '<tr>'+
      ...
      '<td class="num">'+r.activeCount+(r.balloonCount>0?' <span style="color:#c9952b;font-size:10px">('+r.balloonCount+' balloon)</span>':'')+'</td>'+
      '<td class="num" style="color:#2ecc71">'+fmt(r.nrIncome)+'</td>'+
      '<td class="num" style="color:#c9952b;font-weight:'+(r.balloonIncome>0?'600':'normal')+'">'+(r.balloonIncome>0?fmt(r.balloonIncome):'-')+'</td>'+
